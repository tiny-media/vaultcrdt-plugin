...
- Search first with `memory-vault find <query...>` before changing architecture, workflow, or coding rules.
- Read exact items with `memory-vault show <id-or-path>` when a result looks relevant.
- Write memory only when the knowledge is clearly reusable across sessions.
- Use typed writes with `memory-vault add decision|convention|procedure|mistake|plan ...`.
- After writing new memory, run `memory-vault reindex` and regenerate the views.

## Current high-salience memory
### Decisions
