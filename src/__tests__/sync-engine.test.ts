...
vi.mock('obsidian', () => ({
  ...
  Notice: class { constructor(msg: string, _timeout?: number) { notices.push(msg); } },
}));
...
describe('SyncEngine', () => {
  ...
  describe('auth', () => {
    ...
    it('uses /auth/device when a device key is set, never /auth/verify', async () => {
      engine = new SyncEngine(makeApp(), makeSettings({ deviceKey: 'dk-1' }));
      await engine.start();
      const urls = mockRequestUrl.mock.calls.map((c: any[]) => c[0]?.url);
      expect(urls).toContain('http://localhost:3737/auth/device');
      expect(urls).not.toContain('http://localhost:3737/auth/verify');
      const call = mockRequestUrl.mock.calls.find((c: any[]) => c[0].url.includes('/auth/device'))!;
      expect(JSON.parse(call[0].body as string)).toEqual({
        vault_id: 'vault-abc', peer_id: 'peer-test', device_key: 'dk-1',
      });
    });

    it('surfaces the auth-rejected notice on 401 from /auth/device', async () => {
      engine = new SyncEngine(makeApp(), makeSettings({ deviceKey: 'dk-bad' }));
      mockRequestUrl.mockRejectedValueOnce(Object.assign(new Error('nope'), { status: 401 }));
      await expect(engine.start()).rejects.toThrow();
      expect(notices.filter(n => n.includes('rejected this device'))).toHaveLength(1);
    });

    it('opens WebSocket on /ws WITHOUT token in the URL', async () => {
      await engine.start();
      ...
      expect(MockWebSocket).toHaveBeenCalledWith(expect.stringContaining('peer_id='));
    });
  ...
  describe('socket authentication', () => {
    it('first frame after open is auth with token and protocol_version', async () => {
      await engine.start();
      mockEncode.mockClear();
      mockWsInstance.onopen!({} as Event);
      expect(mockEncode.mock.calls[0][0]).toEqual({ type: 'auth', token: 'test-token', protocol_version: 1 });
      expect(MockWebSocket).toHaveBeenCalledWith(expect.not.stringContaining('token='));
      expect(MockWebSocket).toHaveBeenCalledWith(expect.stringContaining('/ws?'));
    ...
    it('auth_ok gates heartbeat and onInitialSync', async () => {
      ...
      try {
        ...
        expect(engine.onInitialSync).not.toHaveBeenCalled();
        expect(mockEncode.mock.calls.some(c => c[0].type === 'ping')).toBe(false);
        fireMessage({ type: 'auth_ok', protocol_version: 1 });
        expect(engine.onInitialSync).toHaveBeenCalledTimes(1);
        vi.advanceTimersByTime(30_000);
        expect(mockEncode.mock.calls.some(c => c[0].type === 'ping')).toBe(true);
      } finally { await engine.stop(); vi.useRealTimers(); }
    });

    it('close before auth_ok: error status, friendly notice, backoff capped', async () => {
      vi.useFakeTimers();
      try {
        const cb = vi.fn();
        ...
        for (let i = 0; i < 8; i++) {
          mockWsInstance.onopen!({} as Event);
          mockWsInstance.onclose!({ code: 1008, reason: 'auth_invalid' } as CloseEvent);
          await flush();
          vi.advanceTimersByTime(31_000);
          await flush();
          expect(MockWebSocket.mock.calls.length).toBe(i + 2);
          expect((engine as any).backoffMs).toBeLessThanOrEqual(30_000);
        }
        expect(cb).toHaveBeenCalledWith('error');
        expect(notices.filter(n => n.includes('rejected this device'))).toHaveLength(1);
        expect((engine as any).backoffMs).toBe(30_000);
      } finally { await engine.stop(); vi.useRealTimers(); }
    });

    it.each([[2, 'CLIENT TOO OLD'], [0, 'SERVER NEEDS UPDATE']])('protocol mismatch: friendly notice once, reconnect keeps backoff (%s)', async (server, text) => {
      vi.useFakeTimers();
      try {
        await engine.start();
        for (let i = 0; i < 2; i++) {
          mockWsInstance.onopen!({} as Event);
          expect(() => fireMessage({ type: 'error', code: 'protocol_version_mismatch', message: `server=${server} client=1` })).not.toThrow();
          mockWsInstance.onclose!({ code: 1008, reason: 'protocol_version_mismatch' } as CloseEvent);
          vi.advanceTimersByTime(31_000);
          await flush();
          expect((engine as any).backoffMs).toBe(1000 * 2 ** (i + 1));
        }
        expect(notices.filter(n => n.includes(String(text)))).toHaveLength(1);
      } finally { await engine.stop(); vi.useRealTimers(); }
    });
  });
  ...
  describe('initialSync — server-only doc', () => {
    ...
    it('downloads server-only doc even when orphan .loro has version>0', async () => {
      // Orphan local CRDT state without a vault file must not block download.
      mockVault.getMarkdownFiles.mockReturnValue([]);
      ...
      fireMessage({
        type: 'doc_list',
        docs: [{ doc_uuid: 'orphan.md', updated_at: '2026-03-16T00:00:00Z', vv_json: '{}' }],
        tombstones: [],
      });
      await flush();
      ...
      const syncStartCalls = mockEncode.mock.calls.filter(
        (c: any[]) => c[0]?.type === 'sync_start' && c[0]?.doc_uuid === 'orphan.md'
      );
      expect(syncStartCalls.length).toBe(1);
      expect(mockVault.create).toHaveBeenCalledWith('orphan.md', 'server content');
    });
  });

  describe('initialSync — startup overwrite guard (U44b)', () => {
    let file: TFile;
    const conflicts = () => mockVault.create.mock.calls.filter(c => c[0].includes('conflict'));

    beforeEach(() => {
      file = Object.assign(Object.create(TFile.prototype), { path: 'startup.md' });
      mockVault.getMarkdownFiles.mockReturnValue([file]);
      mockVault.getAbstractFileByPath.mockImplementation((path: string) => {
        if (path === file.path) return file;
        return mockVault.create.mock.calls.some(c => c[0] === path) ? { path } : null;
      });
      mockVault.read.mockResolvedValueOnce('local').mockResolvedValue('local\nPROBE');
      mockDocInstance.version.mockReturnValue(5);
      mockDocInstance.text_matches.mockReturnValue(true);
      mockDocInstance.export_vv_json.mockReturnValue('{"peer-a":5}');
      mockDocInstance.get_text.mockReturnValue('server');
    });

    const sync = async (serverVV = '{"peer-a":6}') => {
      await engine.start();
      engine.traceEditorChange(file.path, {});
      const pending = engine.initialSync();
      fireMessage({
        type: 'doc_list',
        docs: [{ doc_uuid: file.path, updated_at: '2026-09-05T00:00:00Z', server_vv: new TextEncoder().encode(serverVV) }],
        tombstones: [],
      });
      await flush(50);
      fireMessage({
        type: 'sync_delta', doc_uuid: file.path, delta: new Uint8Array(64),
        server_vv: new TextEncoder().encode(serverVV),
      });
      await pending;
    };

    it('preserves an unaccounted disk edit before the merged write (RUN-C)', async () => {
      // Quiet mode (design §E): the conflict goes to the inbox, not to a Notice.
      const inboxed: Array<Record<string, unknown>> = [];
      engine.inbox = { add: (e) => { inboxed.push({ ...e }); } };
      await sync();
      expect(inboxed).toHaveLength(1);
      expect(inboxed[0].kind).toBe('conflict');
      expect(conflicts()).toHaveLength(1);
      expect(conflicts()[0][1]).toBe('local\nPROBE');
      expect(mockVault.modify).toHaveBeenCalledExactlyOnceWith(file, 'server');
      expect(engine.getStartupTraceReport()).toContain('startup.overwrite-refused | path=startup.md');
      expect(notices).toHaveLength(0);
      expect(mockVault.create.mock.invocationCallOrder[0]).toBeLessThan(mockVault.modify.mock.invocationCallOrder[0]);
    });

    it('does not copy already-accounted local text', async () => {
      mockVault.read.mockReset().mockResolvedValue('local');
      await sync();
      expect(conflicts()).toHaveLength(0);
      expect(engine.getStartupTraceReport()).not.toContain('startup.overwrite-refused');
      expect(mockVault.modify).toHaveBeenCalledExactlyOnceWith(file, 'server');
    });

    it('leaves open-editor writes unchanged despite stale disk text', async () => {
      const editor = {
        getValue: vi.fn().mockReturnValue('editor text'),
        setValue: vi.fn(),
      ...
      const view = Object.assign(Object.create(MockMarkdownView.prototype), { file, editor });
      engine = new SyncEngine(makeApp([{ view }]), makeSettings());
      mockVault.read.mockReset().mockResolvedValue('stale');
      await sync();
      expect(conflicts()).toHaveLength(0);
      expect(engine.getStartupTraceReport()).not.toContain('startup.overwrite-refused');
      expect(editor.setValue).toHaveBeenCalledExactlyOnceWith('server');
    });

    it('preserves both the Phase-3 snapshot and the fresher disk text', async () => {
      mockVault.read.mockReset().mockResolvedValueOnce('Local offline edit').mockResolvedValue('Local offline edit + PROBE');
      await sync('{"peer-b":3}');
      expect(conflicts().map(c => c[1])).toEqual(['Local offline edit', 'Local offline edit + PROBE']);
      expect(conflicts()[1][0]).not.toBe(conflicts()[0][0]);
      expect(engine.getStartupTraceReport()).toContain('startup.overwrite-refused');
      expect(mockVault.modify).toHaveBeenCalledExactlyOnceWith(file, 'server');
    });

    it('preserves a local file created during a server-only download', async () => {
      mockVault.getMarkdownFiles.mockReturnValue([]);
      mockDocInstance.version.mockReturnValue(0);
      mockVault.read.mockReset().mockResolvedValue('typed during window');
      await sync();
      expect(conflicts()).toHaveLength(1);
      expect(conflicts()[0][1]).toBe('typed during window');
      expect(engine.getStartupTraceReport()).toContain('startup.overwrite-refused');
      expect(mockVault.modify).toHaveBeenCalledExactlyOnceWith(file, 'server');
    });
  });
  ...
  describe('initialSync — content-hash fast-path', () => {
    it('skips disk read and CRDT sync when VV and content hash both match', async () => {
      ...
      const { fnv1aHash64 } = await import('../conflict-utils');
      const expectedHash = fnv1aHash64('hello world');
    ...
    it('takes the clean skip when the path is clean and unacked/pending are empty', async () => {
      const tfile = Object.create(TFile.prototype);
      tfile.path = 'cached-eq.md';
      mockVault.getMarkdownFiles.mockReturnValue([tfile]);
      mockVault.read.mockResolvedValue('steady state');
      ...
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        _version: 5,
        'cached-eq.md': { vv: '{"peer1":10}', contentHash: expectedHash },
      }));
      mockAdapter.list.mockResolvedValue({ files: [], folders: [] });

      await engine.start();
      engine.traceEditorChange('cached-eq.md', {});
      // Empty refusal sets: nothing sent-but-unacked, nothing debounce-pending.
      expect((engine as any).push.hasUnackedEdit('cached-eq.md')).toBe(false);
      expect((engine as any).push.hasPendingEdits('cached-eq.md')).toBe(false);
      const syncPromise = engine.initialSync();
      await flush();

      fireMessage({
        type: 'doc_list',
        docs: [{ doc_uuid: 'cached-eq.md', updated_at: '2026-03-16T00:00:00Z', server_vv: new TextEncoder().encode('{"peer1":10}') }],
        tombstones: [],
      });
      ...
      expect(report).toContain('initial-sync.vv-clean-skip');
      expect(report).not.toContain('initial-sync.clean-skip-refused');
    });

    it('clean skip no longer consults the local .loro VV — an ahead VV alone does not refuse', async () => {
      const tfile = Object.create(TFile.prototype);
      tfile.path = 'cached-ahead.md';
      mockVault.getMarkdownFiles.mockReturnValue([tfile]);
      mockVault.read.mockResolvedValue('steady state');
      ...
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        _version: 5,
        'cached-ahead.md': { vv: '{"peer1":10}', contentHash: expectedHash },
      }));
      mockAdapter.list.mockResolvedValue({ files: [], folders: [] });
      // Local .loro doc is AHEAD of the (stale) server VV: peer1:12 > 10 —
      // e.g. a pre-restart zombie-phase push whose ops live only locally.
      // The VV consultation was removed (26e7949 counter-review): an ahead
      // VV alone no longer refuses — only sent-unacked, debounce-pending or
      // the persisted dirty mark do. Pre-restart losses carry the dirty mark.
      mockDocInstance.export_vv_json.mockReturnValue('{"peer1":12}');

      await engine.start();
      engine.traceEditorChange('cached-ahead.md', {});
      const syncPromise = engine.initialSync();
      await flush();

      fireMessage({
        type: 'doc_list',
        docs: [{ doc_uuid: 'cached-ahead.md', updated_at: '2026-03-16T00:00:00Z', server_vv: new TextEncoder().encode('{"peer1":10}') }],
        tombstones: [],
      });
    ...
    it('refuses the clean skip when the push is sent-but-unacked — lost zombie-phase edit is synced', async () => {
      vi.useFakeTimers();
      try {
        const tfile = Object.create(TFile.prototype);
        tfile.path = 'zombie.md';
        mockVault.getMarkdownFiles.mockReturnValue([tfile]);
        // An open editor leaf holds the zombie-phase edit; the cached hash
        // is the PRE-edit hash. With cached.vv == serverVV (both stale), only
        // the refused clean skip stands between the edit and permanent burial
        // — sentUnacked is the in-session witness that ops are pending (U42).
        const mockEditor = {
          getValue: vi.fn().mockReturnValue('zombie phase edit'),
          setValue: vi.fn(),
          getCursor: vi.fn().mockReturnValue({ line: 0, ch: 0 }),
          setCursor: vi.fn(),
          lastLine: vi.fn().mockReturnValue(0),
        ...
        const staleHash = fnv1aHash64('hello world');
        mockAdapter.exists.mockResolvedValue(true);
        mockAdapter.read.mockResolvedValue(JSON.stringify({
          _version: 5,
          'zombie.md': { vv: '{"peer1":10}', contentHash: staleHash },
        }));
        mockAdapter.list.mockResolvedValue({ files: [], folders: [] });
        mockDocInstance.version.mockReturnValue(5);
        mockDocInstance.text_matches.mockReturnValue(false);
        mockDocInstance.get_text.mockReturnValue('zombie phase edit');
        mockDocInstance.export_vv_json.mockReturnValue('{"peer1":12}');
        mockDocInstance.export_delta_since_vv_json.mockReturnValue(new Uint8Array(32));

        await engine.start();

        // Drive a REAL push: the debounce fires, sync_push goes out, and the
        // path lands in sentUnacked — the push went into a half-dead socket,
        // sent client-side, never acknowledged.
        engine.onFileChanged('zombie.md');
        vi.advanceTimersByTime(350);
        await flush();
        expect((engine as any).push.hasPendingEdits('zombie.md')).toBe(false);
        expect((engine as any).push.hasUnackedEdit('zombie.md')).toBe(true);

        // Isolate the unacked predicate: drop the (also-set) persisted dirty
        // mark so ONLY sentUnacked can refuse the skip.
        (engine as any).startupDirty.clear('zombie.md');
        expect((engine as any).startupDirty.has('zombie.md')).toBe(false);
        ...
        fireMessage({
          type: 'doc_list',
          docs: [{ doc_uuid: 'zombie.md', updated_at: '2026-03-16T00:00:00Z', server_vv: new TextEncoder().encode('{"peer1":10}') }],
          tombstones: [],
        });

        await flush(50);

        // Skip refused → the path is processed as before: full overlap sync,
        // and the refusal is visible in the startup trace.
        const report = engine.getStartupTraceReport();
        expect(report).toContain('initial-sync.clean-skip-refused');
        expect(report).not.toContain('initial-sync.vv-clean-skip');

        const syncStartCalls = mockEncode.mock.calls.filter(
          (c: any[]) => c[0]?.type === 'sync_start' && c[0]?.doc_uuid === 'zombie.md'
        );
        expect(syncStartCalls.length).toBe(1);

        // The zombie-phase edit enters the CRDT (editor content picked up) ...
        expect(mockDocInstance.sync_from_disk).toHaveBeenCalledWith('zombie phase edit');
        ...
        // ... and the pending delta is pushed to the server (VV gap: 12 > 10).
        // Two pushes total: the pre-sync debounce push + the VV-gap push.
        const pushCalls = mockEncode.mock.calls.filter(
          (c: any[]) => c[0]?.type === 'sync_push' && c[0]?.doc_uuid === 'zombie.md'
        );
    ...
    it('performs ZERO .loro loads (getOrLoad) on the clean-skip path', async () => {
      const tfile = Object.create(TFile.prototype);
      tfile.path = 'cached-zero.md';
      mockVault.getMarkdownFiles.mockReturnValue([tfile]);
      mockVault.read.mockResolvedValue('never read');

      const expectedHash = fnv1aHash64('steady state');
      mockAdapter.exists.mockResolvedValue(true);
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        _version: 5,
        'cached-zero.md': { vv: '{"peer1":10}', contentHash: expectedHash },
      }));
      mockAdapter.list.mockResolvedValue({ files: [], folders: [] });

      await engine.start();
      // Regression guard (26e7949 counter-review): the clean skip must not
      // touch the docs manager at all — every getOrLoad here would mean a
      // serial .loro read + WASM import per warm-started doc (the U44a
      // regression returning as local I/O).
      const getOrLoadSpy = vi.spyOn((engine as any).docs, 'getOrLoad');
      engine.traceEditorChange('cached-zero.md', {});
      const syncPromise = engine.initialSync();
      await flush();

      fireMessage({
        type: 'doc_list',
        docs: [{ doc_uuid: 'cached-zero.md', updated_at: '2026-03-16T00:00:00Z', server_vv: new TextEncoder().encode('{"peer1":10}') }],
        tombstones: [],
      });

      await syncPromise;

      // Positive control: the skip branch actually ran ...
      const report = engine.getStartupTraceReport();
      expect(report).toContain('initial-sync.vv-clean-skip');
      // ... while performing zero .loro loads and zero disk reads.
      expect(getOrLoadSpy).not.toHaveBeenCalled();
      expect(mockVault.read).not.toHaveBeenCalled();
    });
    ...
    it('rejects a legacy v4 (32-bit hash) cache → full re-sync on next start', async () => {
      ...
      tfile.path = 'clean.md';
      mockVault.getMarkdownFiles.mockReturnValue([tfile]);
      mockVault.read.mockResolvedValue('clean text');
      mockDocInstance.version.mockReturnValue(5);

      // v4 cache held 32-bit numeric hashes; loading it could silently skip a
      // real change on a hash collision. It must be discarded → full re-sync.
    ...
    it('falls through to full sync when server VV differs', async () => {
      ...
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        _version: 5,
        'edited.md': { vv: '{"peer1":5}', contentHash: 'deadbeefdeadbeef' },
      }));
      mockAdapter.list.mockResolvedValue({ files: [], folders: [] });
    ...
    it('persists the downloaded server text hash so the next sync can skip server-only docs', async () => {
      ...
      expect(vvCache['remote.md']).toEqual({
        vv: '{"peer1":1}',
        contentHash: fnv1aHash64('server content'),
      });
    ...
    it('persists the final merged text hash instead of the stale pre-sync local text', async () => {
      ...
      expect(vvCache['overlap.md']).toEqual({
        vv: '{"peer1":2}',
        contentHash: fnv1aHash64('server merged'),
      });
    ...
    it('marks locally changed paths dirty and clears them after startup verification', async () => {
      vi.useFakeTimers();
      try {
      const { fnv1aHash64 } = await import('../conflict-utils');
      const { textFiles } = installVirtualStateStore();
      ...
      textFiles.set('.obsidian/plugins/vaultcrdt/state/vv-cache.json', JSON.stringify({
        ...
        'dirty.md': {
          vv: '{"peer1":10}',
          contentHash: fnv1aHash64('same text'),
        },
      }));
      ...
      await flush();
      // Fire the debounce before syncing: a debounce-pending path refuses
      // the clean skip by design, but this test is about the dirty mark.
      // With no open editor the fired debounce is a no-op push-wise.
      vi.advanceTimersByTime(350);
      await flush();
      expect((engine as any).push.hasPendingEdits('dirty.md')).toBe(false);
  ...
  describe('initialSync — parallel overlapping reads', () => {
    it('T-P1: reads each overlapping file exactly once', async () => {
      ...
      const hash = fnv1aHash64(content);
      ...
      await engine.start();
      for (const f of files) engine.onFileChanged(f.path);
      await flush();
      const syncPromise = engine.initialSync();
      await flush();
    ...
    it('T-P3: one hash mismatch triggers exactly one requestSyncStart', async () => {
      ...
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        ...
        'm2.md': { vv: '{"p":1}', contentHash: '0000000000000000' },
  ...
  describe('broadcast vv-cache', () => {
    const broadcast = (path: string) => ({
      doc_uuid: path, delta: new Uint8Array(8),
      server_vv: new TextEncoder().encode('{"remote":2}'),
    });

    it.each([false, true])('caches imported text and server VV (surgical=%s)', async (surgical) => {
      vi.useFakeTimers();
      try {
        const internal = engine as any;
        mockDocInstance.export_vv_json.mockReturnValue('{"remote":2,"local":1}');
        mockDocInstance.get_text.mockReturnValue('imported text');
        mockDocInstance.import_and_diff.mockReturnValue('[]');
        vi.spyOn(internal.editor, 'applyDiffToEditor').mockReturnValue(surgical);
        vi.spyOn(internal.editor, 'writeToVault').mockResolvedValue(undefined);
        await internal.onDeltaBroadcast(broadcast('received.md'));
        expect(internal.vvCache.get('received.md')).toEqual({
          vv: '{"remote":2}', contentHash: fnv1aHash64('imported text'),
        });
      } finally { await engine.stop(); vi.useRealTimers(); }
    });

    it.each(['close', 'stop'])('batches broadcasts for 5 s and flushes on %s', async (end) => {
      vi.useFakeTimers();
      try {
        await engine.start();
        const internal = engine as any;
        mockDocInstance.export_vv_json.mockReturnValue('{"remote":2}');
        mockDocInstance.get_text.mockReturnValue('imported text');
        vi.spyOn(internal.editor, 'writeToVault').mockResolvedValue(undefined);
        const save = vi.spyOn(internal.docs, 'saveVVCache').mockResolvedValue(undefined);
        for (let i = 0; i < 10; i++) {
          await internal.onDeltaBroadcast(broadcast(`${i}.md`));
        ...
        expect(save.mock.calls[0][0]).toHaveProperty('size', 10);
        await internal.onDeltaBroadcast(broadcast('final.md'));
        if (end === 'close') {
    ...
    it('retains dirty cache after a failed timer write for the unload flush', async () => {
      vi.useFakeTimers();
      try {
        const internal = engine as any;
        mockDocInstance.export_vv_json.mockReturnValue('{"remote":2}');
        vi.spyOn(internal.editor, 'writeToVault').mockResolvedValue(undefined);
        const save = vi.spyOn(internal.docs, 'saveVVCache')
          .mockRejectedValueOnce(new Error('write failed')).mockResolvedValue(undefined);
        await internal.onDeltaBroadcast(broadcast('retry.md'));
        await vi.advanceTimersByTimeAsync(5000);
        expect(save).toHaveBeenCalledTimes(1);
        await engine.stop();
        expect(save).toHaveBeenCalledTimes(2);
        expect(internal.vvCacheDirty).toBe(false);
      } finally { vi.useRealTimers(); }
    });

    it('unload waits for an active write and persists newer broadcasts without overlap', async () => {
      vi.useFakeTimers();
      try {
        const internal = engine as any;
        mockDocInstance.export_vv_json.mockReturnValue('{"remote":2}');
        ...
        const save = vi.spyOn(internal.docs, 'saveVVCache')
          // eslint-disable-next-line @typescript-eslint/no-misused-promises -- saveVVCache is async; the spy type erases that
          .mockImplementationOnce((): Promise<void> => blocked)
          .mockResolvedValue(undefined);
        await internal.onDeltaBroadcast(broadcast('first.md'));
        await vi.advanceTimersByTimeAsync(5000);
        await internal.onDeltaBroadcast(broadcast('second.md'));
    ...
    it('does not cache an import when both import methods fail', async () => {
      const internal = engine as any;
      mockDocInstance.import_and_diff.mockImplementationOnce(() => { throw new Error('bad delta'); });
      mockDocInstance.import_snapshot.mockImplementationOnce(() => { throw new Error('bad snapshot'); });
      await internal.onDeltaBroadcast(broadcast('bad.md'));
      expect(internal.vvCache.has('bad.md')).toBe(false);
    });

    it('caches the adopted disjoint snapshot', async () => {
      vi.useFakeTimers();
      try {
        const internal = engine as any;
        vi.spyOn(internal.editor, 'readCurrentContent').mockReturnValue('');
        vi.spyOn(internal.editor, 'writeToVault').mockResolvedValue(undefined);
        vi.spyOn(internal, 'requestSyncStart').mockResolvedValue({
          delta: new Uint8Array(8), serverVV: '{"remote":2}',
        });
        mockDocInstance.get_text.mockReturnValue('adopted text');
        await internal.resolveDisjointHistory('adopted.md', 'create_conflict');
        expect(internal.vvCache.get('adopted.md')).toEqual({
          vv: '{"remote":2}', contentHash: fnv1aHash64('adopted text'),
        });
      } finally { await engine.stop(); vi.useRealTimers(); }
    });
  });
  ...
  // ── case-insensitive tombstones ───────────────────────────────────────────

  it('does not trash a case-only tombstone when an active server doc exists', async () => {
    const tfile = Object.create(TFile.prototype);
    tfile.path = 'Foo.md';
    mockVault.getMarkdownFiles.mockReturnValue([tfile]);
    mockVault.read.mockResolvedValue('content');
    mockVault.getAbstractFileByPath.mockImplementation((path: string) => {
      if (path === 'foo.md') return tfile;
      return null;
    });
    mockDocInstance.version.mockReturnValue(1);
    mockDocInstance.text_matches.mockReturnValue(true);
    mockDocInstance.export_vv_json.mockReturnValue('{"peer-test":1}');
    ...
    fireMessage({
      type: 'doc_list',
      docs: [{ doc_uuid: 'Foo.md', updated_at: '2026-06-06T00:00:00Z', server_vv: new TextEncoder().encode('{"peer-test":1}') }],
      tombstones: ['foo.md'],
    });
    await flush();
    fireMessage({
      ...
      server_vv: new TextEncoder().encode('{"peer-test":1}'),
    });
    await syncPromise;
  ...
  describe('onFileRenamed', () => {
    ...
    it('case-only rename sends doc_delete for old path and push for new path', async () => {
      ...
      const removeSpy = vi.spyOn(docs, 'removeAndClean');

      engine.onFileRenamed('foo.md', 'Foo.md', 'content');
      await flush();

      const deleteCalls = mockEncode.mock.calls.filter(
        (c: any[]) => c[0]?.type === 'doc_delete' && c[0]?.doc_uuid === 'foo.md'
      );
      expect(deleteCalls.length).toBe(1);

      const pushCalls = mockEncode.mock.calls.filter(
        (c: any[]) => c[0]?.type === 'sync_push' && c[0]?.doc_uuid === 'Foo.md'
      );
      expect(pushCalls.length).toBe(1);

      // Local CRDT state must survive: no removeAndClean, same instance under newPath
      expect(removeSpy).not.toHaveBeenCalled();
      expect(docs.get('Foo.md')).toBe(doc);
      expect(docs.get('foo.md')).toBeUndefined();
    });
  });
  ...
  describe('concurrent create conflict fork', () => {
    ...
    it('disjoint VV + same text → adopt server, no conflict, no merge', async () => {
      ...
      // the conflict-storm audit notes (local): when local CRDT history and server
    ...
    it('disjoint VV + same text must NOT call sync_from_disk to merge', async () => {
      ...
      // text in a large real-world vault.
    ...
    it('T-C5: vv-cache hash reflects CRDT text after startup skip', async () => {
      ...
      // Hash must be a real 64-bit hex hash (derived from CRDT text, not the stale disk)
      expect(cache['tc5.md'].contentHash).not.toBe('');
      expect(cache['tc5.md'].contentHash).toMatch(/^[0-9a-f]{16}$/);
  ...
  describe('echo guard', () => {
    it('stores a 64-bit content hash in lastRemoteWrite, never the text', async () => {
      ...
      const stored = (engine as any).lastRemoteWrite.get('hash.md');
      expect(stored).toBe(fnv1aHash64('remote content'));
  ...
  describe('doc_deleted broadcast', () => {
    it('keeps the file and registers a recreate intent when a push debounce is pending', async () => {
      vi.useFakeTimers();
      const mockFile = Object.create(TFile.prototype);
      mockVault.getAbstractFileByPath.mockReturnValue(mockFile);

      await engine.start();
      engine.onFileChanged('kept.md'); // schedules the 300 ms debounce → pending edits
      ...
      expect(journalWrites[journalWrites.length - 1][1]).toContain('kept.md');
      // No doc_delete may be sent for a remote-originated tombstone.
      expect(mockEncode.mock.calls.filter((c: any[]) => c[0]?.type === 'doc_delete')).toHaveLength(0);
  ...
  describe('doc_deleted keep-guard — sent-but-unacked pushes', () => {
    const mockEditor = {
      getValue: vi.fn().mockReturnValue('hello'),
      setValue: vi.fn(),
      getCursor: vi.fn().mockReturnValue({ line: 0, ch: 0 }),
    ...
    const makeUnackedLeaf = (path: string) => ({
      view: Object.assign(Object.create(MockMarkdownView.prototype), {
        file: { path },
        editor: mockEditor,
      }),
    });
    const syncPushes = (path: string) => mockEncode.mock.calls.filter(
      (c: any[]) => c[0]?.type === 'sync_push' && c[0]?.doc_uuid === path,
    );

    /** Fire the debounce for `path` and wait until the push was sent. */
    const firePush = async (path: string, content: string) => {
      mockEditor.getValue.mockReturnValue(content);
      engine.onFileChanged(path);
      vi.advanceTimersByTime(350);
      await flush();
    };

    it('keeps the file (recreate intent) when the last push is sent-but-unacked', async () => {
      vi.useFakeTimers();
      try {
        const mockFile = Object.create(TFile.prototype);
        mockVault.getAbstractFileByPath.mockReturnValue(mockFile);
        engine = new SyncEngine(makeApp([makeUnackedLeaf('kept.md')]), makeSettings());
        await engine.start();

        await firePush('kept.md', 'typed content');
        expect(syncPushes('kept.md')).toHaveLength(1);
        // Defect scenario: the push went into a half-dead socket — sent
        // client-side, never acknowledged, debounce long since fired.
        expect((engine as any).push.hasPendingEdits('kept.md')).toBe(false);
        expect((engine as any).push.hasUnackedEdit('kept.md')).toBe(true);
        ...
        expect((engine as any).push.hasPendingDelete('kept.md')).toBe(true);
        const report = engine.getStartupTraceReport();
        expect(report).toContain('delete.kept-local-edits');
        expect(report).toContain('"unacked":true');
      } finally {
        vi.clearAllTimers();
        vi.useRealTimers();
      }
    ...
    it('proceeds to trash after initial-sync completion cleared the unacked set', async () => {
      vi.useFakeTimers();
      try {
        const mockFile = Object.create(TFile.prototype);
        mockVault.getAbstractFileByPath.mockReturnValue(mockFile);
        ...
        // A successful initial sync reconciles server truth and clears the set.
        const syncPromise = engine.initialSync();
        await flush();
        fireMessage({ type: 'doc_list', docs: [], tombstones: [] });
        await syncPromise;
        expect((engine as any).push.hasUnackedEdit('kept.md')).toBe(false);
        // Post-reconcile: the local doc now matches the editor content, so
        // only the (cleared) unacked set could have kept the guard armed.
        mockDocInstance.text_matches.mockReturnValue(true);
    ...
    it('tracks a sent push as unacked; a second push keeps one entry', async () => {
      vi.useFakeTimers();
      try {
        engine = new SyncEngine(makeApp([makeUnackedLeaf('note.md')]), makeSettings());
        await engine.start();

        await firePush('note.md', 'v1');
        expect(syncPushes('note.md')).toHaveLength(1);
        expect((engine as any).push.hasUnackedEdit('note.md')).toBe(true);
        expect((engine as any).push.sentUnackedCount()).toBe(1);

        await firePush('note.md', 'v2');
        expect(syncPushes('note.md')).toHaveLength(2);
        expect((engine as any).push.sentUnackedCount()).toBe(1);
      } finally {
        vi.clearAllTimers();
        vi.useRealTimers();
    ...
    it('tracks flushPendingEdits and pushDocCreate sends as unacked too', async () => {
      vi.useFakeTimers();
      try {
        const mockFile = Object.create(TFile.prototype);
        mockFile.path = 'note.md';
        ...
        // flushPendingEdits: a broadcast arrives while the debounce is pending.
        mockEditor.getValue.mockReturnValue('hello');
        mockDocInstance.text_matches.mockReturnValue(false);
        engine.onFileChanged('note.md');
        mockDocInstance.get_text.mockReturnValue('merged result');
        mockDocInstance.import_and_diff.mockReturnValue('');
        fireMessage({ type: 'delta_broadcast', doc_uuid: 'note.md', delta: new Uint8Array(64), peer_id: 'other-peer' });
        await flush(50);
        expect(syncPushes('note.md')).toHaveLength(1);
        expect((engine as any).push.hasUnackedEdit('note.md')).toBe(true);

        // pushDocCreate fallback: export_delta fails → doc_create send.
        (engine as any).push.clearSentUnacked();
        expect((engine as any).push.hasUnackedEdit('note.md')).toBe(false);
        mockDocInstance.export_delta_since_vv_json.mockImplementation(() => {
          throw new Error('boom');
        });
        mockDocInstance.export_snapshot.mockReturnValue(new Uint8Array(64));
        await firePush('note.md', 'recreated');
        expect(
          mockEncode.mock.calls.filter((c: any[]) => c[0]?.type === 'doc_create' && c[0]?.doc_uuid === 'note.md'),
        ).toHaveLength(1);
        expect((engine as any).push.hasUnackedEdit('note.md')).toBe(true);
      } finally {
        vi.clearAllTimers();
  ...
  describe('doc_tombstoned', () => {
    ...
    it('adds a tombstone-edit inbox entry when the local file is gone', async () => {
      ...
      await engine.start();
      fireMessage({ type: 'doc_tombstoned', doc_uuid: 'gone.md' });
      await flush();
  ...
  describe('inbox wiring (design §E)', () => {
    const mockEditor = {
      getValue: vi.fn().mockReturnValue('local edit'),
      setValue: vi.fn(),
      getCursor: vi.fn().mockReturnValue({ line: 0, ch: 0 }),
      setCursor: vi.fn(),
      lastLine: vi.fn().mockReturnValue(0),
    ...
    it('adds a deleted-remote inbox entry when a remote delete is kept for pending local edits', async () => {
    ...
    it('collects a failed-docs inbox entry when an overlapping doc fails to sync', async () => {
      const file = Object.assign(Object.create(TFile.prototype), { path: 'fail.md' });
      mockVault.getMarkdownFiles.mockReturnValue([file]);
      mockVault.getAbstractFileByPath.mockImplementation((p: string) => (p === 'fail.md' ? file : null));
      mockVault.read.mockResolvedValue('local content');
      mockDocInstance.get_text.mockReturnValue('server content');
      await engine.start();
      const add = vi.fn();
      ...
      fireMessage({
        type: 'doc_list',
        docs: [{ doc_uuid: 'fail.md', updated_at: '2026-09-06T00:00:00Z', vv_json: '{}' }],
        tombstones: [],
      });
      await flush();
      mockDocInstance.import_snapshot.mockImplementationOnce(() => { throw new Error('boom'); });
      fireMessage({
        type: 'sync_delta', doc_uuid: 'fail.md', delta: new Uint8Array(64),
        server_vv: new TextEncoder().encode('{"1":1}'),
      });
      await syncPromise;

      expect(add).toHaveBeenCalledWith(expect.objectContaining({ kind: 'failed-docs' }));
    });
  });

  describe('disjoint history resolution', () => {
    const enc = (vv: string) => new TextEncoder().encode(vv);
    const messages = (type: string) => mockEncode.mock.calls.filter(
      (c: any[]) => c[0]?.type === type && c[0]?.doc_uuid === 'note.md',
    );
    let file: TFile;

    beforeEach(() => {
      file = Object.assign(Object.create(TFile.prototype), { path: 'note.md' });
      mockVault.getAbstractFileByPath.mockImplementation((p: string) => p === 'note.md' ? file : null);
      mockVault.read.mockResolvedValue('local text');
      mockDocInstance.get_text.mockReturnValue('server text');
      mockDocInstance.export_vv_json.mockReturnValue('{"peer-b":3}');
    });

    const answerSnapshot = () => fireMessage({
      type: 'sync_delta', doc_uuid: 'note.md', delta: new Uint8Array(64),
      server_vv: enc('{"peer-a":5}'),
    });
    const expectAdoption = () => {
      expect(messages('sync_start')).toHaveLength(1);
      expect(messages('sync_start')[0][0].client_vv).toBeNull();
      expect(mockDocInstance.import_and_diff).not.toHaveBeenCalled();
      expect(mockDocInstance.import_snapshot).toHaveBeenCalled();
      expect(mockVault.create).toHaveBeenCalledExactlyOnceWith(expect.stringContaining('(conflict'), 'local text');
      expect(mockVault.modify).toHaveBeenCalledWith(file, 'server text');
      expect(messages('sync_push')).toHaveLength(0);
    };

    it('delta_broadcast with disjoint server_vv forks and adopts server', async () => {
      const add = vi.fn();
      engine.inbox = { add };
      await engine.start();
      engine.onFileChanged('note.md');
      fireMessage({
        type: 'delta_broadcast', doc_uuid: 'note.md', delta: new Uint8Array(32),
        peer_id: 'a', server_vv: enc('{"peer-a":5}'),
      });
      await flush(50);
      expect((engine as any).push.hasPendingEdits('note.md')).toBe(false);
      answerSnapshot();
      await flush(50);
      expectAdoption();
      expect(messages('doc_create')).toHaveLength(0);
      expect(add).toHaveBeenCalledWith(expect.objectContaining({
        kind: 'disjoint-conflict', path: expect.stringContaining('(conflict'), relatedPath: 'note.md',
      }));
    });

    it('missing server_vv + non-blank local → conflict path, never import', async () => {
      await engine.start();
      fireMessage({ type: 'delta_broadcast', doc_uuid: 'note.md', delta: new Uint8Array(32), peer_id: 'a' });
      await flush(50);
      expect(mockDocInstance.import_and_diff).not.toHaveBeenCalled();
    ...
    it('delta_broadcast with shared history does not fire the guard', async () => {
      mockDocInstance.export_vv_json.mockReturnValue('{"peer-a":2}');
      await engine.start();
      fireMessage({
        type: 'delta_broadcast', doc_uuid: 'note.md', delta: new Uint8Array(32),
        peer_id: 'a', server_vv: enc('{"peer-a":5}'),
      ...
      expect(messages('sync_start').every(c => c[0].client_vv !== null)).toBe(true);
      answerSnapshot(); // Finish the unchanged VV-gap catch-up.
      await flush(50);
    });

    it('create_conflict after local-only initial-sync push forks and adopts server', async () => {
      mockVault.getMarkdownFiles.mockReturnValue([file]);
      mockDocInstance.text_matches.mockReturnValue(true);
      await engine.start();
      const syncPromise = engine.initialSync();
      fireMessage({ type: 'doc_list', docs: [], tombstones: [] });
      await flush(50);
      expect(messages('doc_create')).toHaveLength(1);
      fireMessage({ type: 'create_conflict', doc_uuid: 'note.md' });
      await syncPromise;
      await flush(50);
      answerSnapshot();
      await flush(50);
    ...
    it('create_conflict while idle dispatches immediately', async () => {
      await engine.start();
      fireMessage({ type: 'create_conflict', doc_uuid: 'note.md' });
      await flush(50);
      expect(messages('sync_start')).toHaveLength(1);
    ...
    it('queues create_conflict during initial sync and replays after the run', async () => {
      await engine.start();
      const syncPromise = engine.initialSync();
      fireMessage({ type: 'create_conflict', doc_uuid: 'note.md' });
      await flush(50);
      expect(messages('sync_start')).toHaveLength(0);
      fireMessage({ type: 'doc_list', docs: [], tombstones: [] });
      await flush(50);
      answerSnapshot();
      await syncPromise;
    ...
    it('unknown variant is harmless', async () => {
      await engine.start();
      mockEncode.mockClear();
      expect(() => fireMessage({ type: 'totally_unknown', doc_uuid: 'x.md' })).not.toThrow();
      await flush();
      expect(mockEncode).not.toHaveBeenCalled();
    });
  });

  describe('broadcast queue cap', () => {
    it('discards the queue on overflow and runs exactly one extra initialSync pass', async () => {
      await engine.start();
      const syncPromise = engine.initialSync();
      await flush();
      ...
      expect((engine as any).queuedBroadcasts.length).toBe(0);
      expect((engine as any).broadcastQueueOverflowed).toBe(true);

      fireMessage({ type: 'doc_list', docs: [], tombstones: [] });
      await syncPromise;
      await flush();

      const docListRequests = () =>
        mockEncode.mock.calls.filter((c: any[]) => c[0]?.type === 'request_doc_list').length;
      expect(docListRequests()).toBe(2);

      // Finish the resync pass so no promise is left pending.
      fireMessage({ type: 'doc_list', docs: [], tombstones: [] });
      await flush(50);
      expect(docListRequests()).toBe(2); // bounded: no third pass
    });
  });
  ...
  describe('sync mode', () => {
    it('pull mode downloads server docs but skips local-only push', async () => {
      mockVault.getAbstractFileByPath.mockImplementation((path: string) =>
        path.includes('conflict') ? null : Object.create(TFile.prototype));
      mockVault.getMarkdownFiles.mockReturnValue([{ path: 'local-only.md' }]);
      mockVault.read.mockResolvedValue('local content');
      mockDocInstance.get_text.mockReturnValue('server content');
      mockDocInstance.version.mockReturnValue(0);
  ...
  // ── heartbeat activity deadline (zombie socket) ────────────────────────

  describe('heartbeat activity deadline', () => {
    it('force-closes a zombie socket: pings go out, nothing comes back', async () => {
      vi.useFakeTimers();
      engine.onInitialSync = vi.fn();
      await engine.start();
      openAndAuth();

      // Tick 1 (30 s): silence below deadline → ping sent, no close.
      vi.advanceTimersByTime(30_000);
      expect(mockWsInstance.close).not.toHaveBeenCalled();
      const pings = mockEncode.mock.calls.filter((c: any[]) => c[0]?.type === 'ping');
      expect(pings.length).toBe(1);

      // Tick 2 (60 s): 60 s < 90 s deadline. The throttled Electron renderer
      // stalls its timers >15 s on 30 s ticks — a healthy socket (pongs
      // flowing on the wire) must NOT be killed by that jitter.
      vi.advanceTimersByTime(30_000);
      expect(mockWsInstance.close).not.toHaveBeenCalled();

      // Tick 3 (90 s): 90 s without any server frame ≥ 90 s deadline →
      // socket is force-closed so onclose can run the reconnect path.
      // (The server's 120 s read-idle close remains the backstop.)
      vi.advanceTimersByTime(30_000);
    ...
    it('ACTIVITY_DEADLINE_MS is 90_000 (and HEARTBEAT_MS stays 30_000)', async () => {
      // Guards the fix against accidental regression: the deadline must sit
      // above the measured >15 s renderer timer stalls on 30 s heartbeat
      // ticks and below the server's 120 s read-idle backstop close.
      const { readFileSync } = await import('node:fs');
      const source = readFileSync(new URL('../sync-engine.ts', import.meta.url), 'utf8');
      const deadline = source.match(/const ACTIVITY_DEADLINE_MS = (\d[\d_]*);/);
      const heartbeat = source.match(/const HEARTBEAT_MS = (\d[\d_]*);/);
      expect(deadline).not.toBeNull();
      expect(heartbeat).not.toBeNull();
      expect(Number(deadline![1].replace(/_/g, ''))).toBe(90_000);
      expect(Number(heartbeat![1].replace(/_/g, ''))).toBe(30_000);
    });

    it('keeps the socket open while server messages keep arriving', async () => {
      vi.useFakeTimers();
      engine.onInitialSync = vi.fn();
      await engine.start();
      openAndAuth();

      // Server answers every ping → activity timestamp keeps refreshing.
      for (let i = 0; i < 5; i++) {
        vi.advanceTimersByTime(30_000);
    ...
    it('does not ping or close after stop()', async () => {
      vi.useFakeTimers();
      engine.onInitialSync = vi.fn();
      await engine.start();
      openAndAuth();
  ...
  describe('initialSync error recovery', () => {
    ...
    it('does not set connected or accept vault events when runInitialSync throws', async () => {
      await engine.start();
      const statuses: string[] = [];
      engine.statusCallback = (s) => statuses.push(s);

      const syncPromise = engine.initialSync();
      await flush();
      // Fail the in-flight doc_list wait without tearing down the socket
      (engine as any).promises.reject('doc_list', new Error('forced fail'));
      await expect(syncPromise).rejects.toThrow('forced fail');
      await flush();

      expect(statuses).not.toContain('connected');
      expect(statuses).toContain('error');
      expect((engine as any).acceptVaultChangeEvents).toBe(false);
    });

    it('skips re-entry while initialSync is already running', async () => {
      await engine.start();
      const first = engine.initialSync();
      await flush();
      // Second call while first is awaiting doc_list must no-op
      await engine.initialSync();
      const docListCalls = mockEncode.mock.calls.filter(
        (c: any[]) => c[0]?.type === 'request_doc_list'
      );
      expect(docListCalls.length).toBe(1);

      fireMessage({ type: 'doc_list', docs: [], tombstones: [] });
      await first;
    });

    it('continues overlapping sync when one doc requestSyncStart fails', async () => {
      mockVault.getMarkdownFiles.mockReturnValue([
        { path: 'bad.md' },
        { path: 'good.md' },
      ]);
      ...
      mockDocInstance.text_matches.mockReturnValue(true);
      mockDocInstance.version.mockReturnValue(1);
      mockDocInstance.export_vv_json.mockReturnValue('{"1":1}');
      mockDocInstance.get_text.mockReturnValue('local');
      ...
      fireMessage({
        type: 'doc_list',
        docs: [
          { doc_uuid: 'bad.md', updated_at: '2026-01-01T00:00:00Z', vv_json: '{"9":9}' },
          { doc_uuid: 'good.md', updated_at: '2026-01-01T00:00:00Z', vv_json: '{"9":9}' },
        ],
        tombstones: [],
      });
      await flush();
      ...
      fireMessage({
        ...
        doc_uuid: 'good.md',
        delta: new Uint8Array(8),
        server_vv: new TextEncoder().encode('{"9":9}'),
      });

      await syncPromise;

      const goodStarts = mockEncode.mock.calls.filter(
        (c: any[]) => c[0]?.type === 'sync_start' && c[0]?.doc_uuid === 'good.md'
      );
      expect(goodStarts.length).toBe(1);
    });
  });

  describe('server error frame', () => {
    it('rejects pending sync_delta when error carries doc_uuid', async () => {
      await engine.start();
      const pending = (engine as any).requestSyncStart('note.md', null);
      await flush();
      fireMessage({
        type: 'error',
        message: 'boom',
    ...
    it('malformed sync_delta without delta rejects pending and does not throw', async () => {
      await engine.start();
      const pending = (engine as any).requestSyncStart('broken.md', null);
      await flush();
      expect(() => {
        fireMessage({
          type: 'sync_delta',
          doc_uuid: 'broken.md',
          // delta missing — would previously throw on .length
          server_vv: new TextEncoder().encode('{}'),
        });
      }).not.toThrow();
  ...
  describe('stale ws onclose after restart', () => {
    it('does not schedule a second connect from an old socket onclose', async () => {
      await engine.start();
      const firstWs = mockWsInstance;
      const firstOnClose = firstWs.onclose;
      expect(firstOnClose).toBeTruthy();

      // Simulate restart replacing the socket
      await engine.restart();
      await flush();
      const connectCallsAfterRestart = MockWebSocket.mock.calls.length;

      // Stale onclose from the old socket must be ignored
      firstOnClose!({} as CloseEvent);
      await flush();

      expect(MockWebSocket.mock.calls.length).toBe(connectCallsAfterRestart);
    });
  });
  ...
  describe('oversized send guard', () => {
    it('does not send when encoded message exceeds 50 MiB', async () => {
      await engine.start();
      mockEncode.mockImplementationOnce(() => new Uint8Array(50 * 1024 * 1024 + 1));
      mockWsInstance.send.mockClear();
      (engine as any).send({ type: 'sync_push', doc_uuid: 'huge.md', delta: new Uint8Array(1), peer_id: 'p' });
