...
describe('SyncEngine — edge cases (S34)', () => {
  ...
  it('online delete then same-path edit sends replace doc_create and clears journal', async () => {
    ...
    await flush();
    engine.onFileChangedImmediate('Untitled.md', 'new file content');
    await flush();

    const deleteCalls = mockEncode.mock.calls.filter(
      (c: any[]) => c[0]?.type === 'doc_delete' && c[0]?.doc_uuid === 'Untitled.md'
    );
    expect(deleteCalls.length).toBe(1);

    const createCalls = mockEncode.mock.calls.filter(
      (c: any[]) => c[0]?.type === 'doc_create' && c[0]?.doc_uuid === 'Untitled.md'
    );
    expect(createCalls.length).toBe(1);
    expect(createCalls[0][0]).toMatchObject({
      type: 'doc_create',
      doc_uuid: 'Untitled.md',
    ...
    expect(lastPayload).not.toContain('Untitled.md');
  });

  it('offline delete then same-path recreate sends replace doc_create, not stale doc_delete', async () => {
    await engine.start();
    ...
    mockWsInstance.readyState = 1;
    mockVault.getMarkdownFiles.mockReturnValue([{ path: 'Untitled.md' }]);
    mockVault.read.mockResolvedValue('new file content');
    // The local file must be findable for the tombstone-pass scenario to be
    // real: without this, trash can never fire and the assertion below
    // would pass vacuously.
    mockVault.getAbstractFileByPath.mockImplementation(
      (p: string) => (p === 'Untitled.md' ? Object.create(TFile.prototype) : null),
    );

    const syncPromise = engine.initialSync();
    await flush();
    fireMessage({ type: 'doc_list', docs: [], tombstones: ['Untitled.md'] });
    await syncPromise;
    ...
    // U37 keep-promise: the file was just re-created via doc_create
    // (replace_tombstone); the tombstone pass of this same run must NOT
    // trash it, even though the run's doc_list snapshot (taken before the
    // recreate push) still lists the path as tombstone-only.
  ...
  it('large merge logged via console.warn', async () => {
    ...
    // Logger prints warn as ONE pre-formatted, redacted string (console
    // redaction, slice A) — the intent of this test is the CONTENT:
    // large merge deltas produce a warn naming path and both lengths.
    expect(warnSpy).toHaveBeenCalledTimes(1);
    const [msg] = warnSpy.mock.calls[0];
    expect(String(msg)).toContain('large merge delta');
    expect(String(msg)).toContain('big.md');
    expect(String(msg)).toContain('1000');
