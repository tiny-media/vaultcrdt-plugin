...
import { ensureDeviceIdentity, resetConnectionState, DEFAULT_SETTINGS, type VaultCRDTSettings } from '../settings';
...
describe('resetConnectionState', () => {
  it('clears the per-device key and onboarding flag', async () => {
    const plugin = {
      settings: makeSettings({ deviceKey: 'dk-1', onboardingComplete: true }),
      syncEngine: { stop: vi.fn(async () => {}), wipeLocalState: vi.fn(async () => {}) },
    };
    await resetConnectionState(plugin as never, false);
    expect(plugin.settings.deviceKey).toBe('');
