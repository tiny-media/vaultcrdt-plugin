...
// ── Slice A: credentials/query/fragment rejection (0.4.3) ───────────────────
describe('normalizeServerUrl rejects secret-bearing URL components', () => {
  it('rejects credentials', () => {
    expect(() => normalizeServerUrl('https://user:pass@sync.example.com'))
      .toThrow('Server URL credentials are not allowed');
  });

  it('rejects query strings', () => {
    expect(() => normalizeServerUrl('https://sync.example.com?token=abc'))
      .toThrow('Server URL query is not allowed');
  });

  it('rejects fragments', () => {
    expect(() => normalizeServerUrl('https://sync.example.com#token'))
      .toThrow('Server URL fragment is not allowed');
  });

  it('validateServerUrl surfaces the same component in its reason', () => {
    const v = validateServerUrl('https://u:p@sync.example.com');
    expect(v.ok).toBe(false);
    expect((v as { reason: string }).reason).toMatch(/credentials are not allowed/);
  });

  it('toHttpBase/toWsBase inherit the rejection', () => {
    expect(() => toHttpBase('https://sync.example.com?x=1')).toThrow('query');
    expect(() => toWsBase('https://sync.example.com#f')).toThrow('fragment');
  });
});
