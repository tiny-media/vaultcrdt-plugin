...
const ready = async () => {};
const snapshot = (c = 1) => JSON.stringify({ v: 2, c, paths: {} });
...
describe('v2 cursor and decisions', () => {
  it.each([false, true])('migrates v1 with entries (backup=%s) and persists v2', async backup => {
    const legacy = JSON.stringify({ v: 1, maxSeq: 90000, paths: { 'a.png': { ...entry, seq: 9000 } } });
    const r = backup ? rig(null, legacy) : rig(legacy);
    expect((await r.index.load()).outcome).toBe(backup ? 'recovered' : 'ok');
    expect(r.index.cursor()).toBe(0);
    expect(r.index.get('a.png')?.seq).toBe(9000);
    r.index.update('a.png', { hydrated: false });
    await r.index.flush();
    const saved = JSON.parse(r.files.get(MAIN)!);
    expect(saved).toMatchObject({ v: 2, c: 0 });
    expect(saved).not.toHaveProperty('maxSeq');
    expect(saved.paths['a.png'].seq).toBe(9000);
  });

  it('round-trips both decisions through verified bak and main', async () => {
    const r = rig(); await r.index.load();
    const del = { kind: 'delete' as const, expectedHash: 'abc', expectedSize: 3, seq: 8, generation: 2 };
    const rep = { kind: 'republish' as const, seq: 9, generation: 3 };
    r.index.update('a.png', { pendingDecision: del });
    r.index.update('b.png', { pendingDecision: rep });
    await r.index.flush();
    r.ops.length = 0;
    r.index.advanceCursor(10); await r.index.flush();
    expect(r.ops).toEqual([`read:${MAIN}`, `write:${BAK}`, `read:${BAK}`, `write:${MAIN}`, `read:${MAIN}`]);
    for (const name of [BAK, MAIN]) {
      const validated = await validateIndexFile(r.files.get(name)!);
  ...
  it.each([
    null, {}, { kind: 'wrong', seq: 1, generation: 1 },
    { kind: 'republish', seq: -1, generation: 1 },
    { kind: 'republish', seq: 1, generation: 1.5 },
    { kind: 'republish', seq: Infinity, generation: 1 },
    { kind: 'republish', seq: 1.5, generation: 1 },
    { kind: 'republish', seq: 1, generation: -1 },
    { kind: 'delete', expectedHash: 'abc', seq: 1, generation: 1 },
    { kind: 'delete', expectedHash: 3, expectedSize: 1, seq: 1, generation: 1 },
    { kind: 'delete', expectedHash: '', expectedSize: 1, seq: 1, generation: 1 },
    { kind: 'delete', expectedHash: 'abc', expectedSize: -1, seq: 1, generation: 1 },
    { kind: 'delete', expectedHash: 'abc', expectedSize: 1.5, seq: 1, generation: 1 },
  ])('invalid decision poisons the whole file: %j', async pendingDecision => {
    const r = rig(JSON.stringify({ v: 2, c: 4, paths: { 'valid.png': { key: 'valid.png', hash: '' }, 'a.png': { ...entry, pendingDecision } } }));
    expect((await r.index.load()).outcome).toBe('poisoned');
    expect(r.index.entries()).toEqual([]);
    expect(r.index.cursor()).toBe(0);
  });

  it.each([NaN, -1, 1.5, Infinity])('invalid cursor poisons: %s', async c => {
    const r = rig(JSON.stringify({ v: 2, c, paths: {} }));
    expect((await r.index.load()).outcome).toBe('poisoned');
    r.store.writeRaw.mockClear();
    r.index.advanceCursor(20); await r.index.flush();
    expect(r.index.cursor()).toBe(0);
    expect(r.store.writeRaw).not.toHaveBeenCalled();
  });

  it('cursor only advances explicitly; lower/equal values do not persist', async () => {
    const r = rig(); await r.index.load();
    r.index.update('a.png', { seq: 100 }); await r.index.flush();
    expect(r.index.cursor()).toBe(1);
    r.index.advanceCursor(4); await r.index.flush();
    r.store.writeRaw.mockClear();
    r.index.advanceCursor(4); r.index.advanceCursor(2); await r.index.flush();
    expect(r.index.cursor()).toBe(4);
    expect(r.store.writeRaw).not.toHaveBeenCalled();
  });
});
...
describe('exclusive load and recovery outcomes', () => {
  it.each([
    ...
    expect(r.index.cursor()).toBe(seq);
    expect(r.index.entries()).toEqual([]);
  ...
  it('backup uses the same whole-snapshot validator and carries paths with its cursor', async () => {
    ...
    expect(r.index.get('a.png')?.hash).toBe(''); expect(r.index.cursor()).toBe(0);
    ...
    expect(r.index.entries()).toEqual([]); expect(r.index.cursor()).toBe(0);
  ...
  it('load waits for a previously queued persist and admits its newest state', async () => {
    ...
    expect(r.index.cursor()).toBe(2); expect(r.files.get(MAIN)).toBe(snapshot(2));
  });
  ...
  it('sticky poison clears only with a valid restored snapshot', async () => {
    ...
    expect(r.index.poisoned()).toBe(false);
    r.index.advanceCursor(11); await r.index.flush();
    expect(JSON.parse(r.files.get(MAIN)!).c).toBe(11);
  });
  it('load is a FIFO task: later persists cannot interleave repair reads/writes', async () => {
    ...
    expect(r.index.cursor()).toBe(9);
    expect(JSON.parse(r.files.get(MAIN)!).c).toBe(9);
  });
  ...
  it('poison overrides newer mutations; queued persist/retry and poisoned mutators write nothing', async () => {
    ...
    r.index.update('c.png', {}); r.index.move('a.png', 'b.png'); r.index.remove('a.png'); r.index.advanceCursor(20);
  ...
  it('restart at a torn repair boundary recovers from frozen backup after old writer stops', async () => {
    ...
    expect(restarted.cursor()).toBe(7); expect(r.files.get(MAIN)).toBe(snapshot(7));
  });
});
...
describe('verified two-write persistence and retry', () => {
  it('rotates exact previous bytes before newest snapshot, with exact readbacks', async () => {
    ...
    r.index.advanceCursor(2); await r.index.flush();
  ...
  it('backup write failure leaves main untouched; flush rejects while failure stands', async () => {
    ...
    r.index.advanceCursor(2); await drained(r.index);
  ...
  it('torn first write has no backup: restart is empty AND poisoned, never fresh', async () => {
    ...
    r.index.advanceCursor(2); await drained(r.index); r.index.dispose();
    expect(r.files.has(BAK)).toBe(false);
    const restarted = r.create();
    expect((await restarted.load(ready)).outcome).toBe('poisoned');
    expect(restarted.entries()).toEqual([]); expect(restarted.cursor()).toBe(0); expect(restarted.poisoned()).toBe(true);
  });
  ...
  it('concurrent flush callers serialize one effective idempotent retry', async () => {
    ...
    r.index.advanceCursor(2); await drained(r.index); r.store.writeRaw.mockClear();
    const gate = deferred();
  ...
  it('single 30s timer repeats without external events and clears on success', async () => {
    ...
    r.index.advanceCursor(2); r.index.advanceCursor(3); await drained(r.index);
  ...
  it('dispose clears timer and pending late failures cannot rearm', async () => {
    ...
    r.index.advanceCursor(2); await drained(r.index); expect(vi.getTimerCount()).toBe(1);
    const gate = deferred();
    r.store.writeRaw.mockImplementationOnce(async () => { await gate.promise; throw Error('late'); });
    r.index.advanceCursor(3); await Promise.resolve(); r.index.dispose(); gate.resolve(); await drained(r.index);
  ...
  it.each(['flush', 'timer'])('FIFO newest snapshot wins across an intervening %s retry', async trigger => {
    ...
    await vi.waitFor(() => expect(r.store.writeRaw).toHaveBeenCalledTimes(2));
    r.index.advanceCursor(3); r.index.advanceCursor(4);
    gate.resolve(); await retry; await r.index.flush();
    expect(r.files.get(MAIN)).toBe(snapshot(4)); expect(r.files.get(BAK)).toBe(snapshot(3)); expect(r.index.cursor()).toBe(4);
    expect(r.store.writeRaw.mock.calls.filter(([n]) => n === MAIN).map(([, s]) => JSON.parse(s).c)).toEqual([2, 3, 4]);
  });
});
