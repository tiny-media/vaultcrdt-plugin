...
import { createDocument } from './wasm-bridge';
...
/**
 * Surface a freshly-created conflict file to the user. Quiet mode (design §E):
 * this never interrupts with a long Notice any more — it files an inbox entry,
 * and the inbox itself shows at most one short, throttled discovery notice.
 */
function notifyConflictCreated(deps: InitialSyncDeps, path: string, conflictPath: string): void {
  deps.inbox?.add({ kind: 'conflict', path: conflictPath, relatedPath: path, note: conflictNoticeMessage(conflictPath) });
}

/** U44b: refuse to bury unaccounted local disk text under server text (cold-start suppression window). */
async function writeServerText(
  deps: InitialSyncDeps, path: string, serverText: string, accountedLocal: string | null,
): Promise<void> {
  const { app, editor, tag } = deps;
  const file = app.vault.getAbstractFileByPath(path);
  if (file instanceof TFile && editor.readCurrentContent(path) === null) {
    const disk = await app.vault.read(file);
    if (disk !== serverText && disk.trim() !== '' && disk !== accountedLocal) {
      const cPath = conflictPath(app, path);
      warn(`${tag} startup overwrite refused — unaccounted local disk change`, { path, conflictPath: cPath });
      await app.vault.create(cPath, disk);
      notifyConflictCreated(deps, path, cPath);
      deps.tracePath('startup.overwrite-refused', path, {
        diskLen: disk.length, serverLen: serverText.length,
        accountedLen: accountedLocal?.length ?? null, conflictPath: cPath,
      });
    }
  }
...
export interface InitialSyncDeps {
  ...
  /** True while a push for `path` was sent but not yet reconciled (U42). */
  hasUnackedEdit: (path: string) => boolean;
  /** True while a debounced editor push for `path` is scheduled but not fired. */
  hasPendingEdits: (path: string) => boolean;
  /** Quiet-mode inbox: conflict/failure events land here instead of a Notice. */
  inbox?: InboxSink | null;
}
...
export async function runInitialSync(
  ...
  const { app, docs, editor, push, lastServerVV, lastRemoteWrite, deletingFromRemote, tag } = deps;
  ...
  const localFiles = (app.vault.getMarkdownFiles()).filter((f) =>
  ...
  const activeServerPathCaseKeys = new Set([...serverDocMap.keys()].map(pathCaseKey));
  ...
  deps.trace('initial-sync.partition', {
    ...
    recreate: recreateFiles.length,
    localOnly: localOnlyFiles.length,
  });
  const totalSteps = serverOnlyUuids.length + overlappingFiles.length + recreateFiles.length + localOnlyFiles.length;
  let stepsDone = 0;
  let changed = 0;
  ...
  const rememberCurrentDocHash = async (path: string): Promise<void> => {
    ...
    contentHashes.set(path, fnv1aHash64(doc.get_text()));
  };
  ...
  if (activeDoc && serverDocMap.has(activeDoc) && localFileMap.has(activeDoc)) {
    ...
    console.debug(`${tag} priority sync complete (${(performance.now() - t0).toFixed(0)}ms)`, { path: activeDoc });
  }
  ...
  if (mode !== 'push') {
    ...
    const downloadOne = async (uuid: string): Promise<void> => {
      ...
      if (existing.version() > 0) {
        // Early-return only when the vault file also exists. An orphan .loro
        // (version>0, no vault file) must not block a server-only download.
        const vaultFile = app.vault.getAbstractFileByPath(uuid);
        if (vaultFile) {
          downloadOk++;
          stepsDone++;
          onProgress?.(stepsDone, totalSteps, changed);
        ...
        // Replace orphan local state before importing the server snapshot.
      ...
      try {
        ...
        if (result) {
          ...
          contentHashes.set(uuid, fnv1aHash64(serverText));
          await writeServerText(deps, uuid, serverText, null);
    ...
    const abortError = wsAbortError as Error | null;
  ...
  console.debug(`${tag} downloads done (${(performance.now() - t0).toFixed(0)}ms): ${downloadOk} ok, ${downloadFail} fail of ${serverOnlyUuids.length}`);
  ...
  for (const file of overlappingFiles) {
    ...
    if (cached && currentServerVV && vvEquals(currentServerVV, cached.vv)) {
      const unacked = deps.hasUnackedEdit(file.path);
      const pendingEdits = deps.hasPendingEdits(file.path);
      if (!dirtyPaths.has(file.path) && !unacked && !pendingEdits) {
      ...
      // cached.vv == serverVV only proves the SERVER side is unchanged. A
      // push this session already sent (zombie-socket loss, U42) or a
      // debounce still pending means local ops may not be on the server —
      // skipping then would bury them permanently behind a stale VV. Both
      // states are tracked in memory by the push handler, so no .loro load
      // is needed here. Pre-restart losses need no predicate: the edit-time
      // dirty mark persists across restarts and routes them through the
      // hash-check tier below (content differs from the cached hash →
      // full sync → the ops are pushed).
      if (!dirtyPaths.has(file.path)) {
        deps.tracePath('initial-sync.clean-skip-refused', file.path, {
          unacked,
          pendingEdits,
        });
        // Fall through to the hash-check/full-sync flow below. The refused
        // path was not planned for a read (the cache said clean), so fetch
        // its content now instead of relying on the pre-read map.
      }

      const effective = effectiveContents.get(file.path) ??
        await readEffectiveLocalContent(app, editor, file);
      const tHash = performance.now();
      const effectiveHash = fnv1aHash64(effective);
  ...
  deps.trace('initial-sync.overlapping.done', {
    ...
    synced: overlappingFiles.length - skippedClean - skippedHashMatch - overlappingFail,
    failed: overlappingFail,
    dirtyRemaining: dirtyPaths.size,
    elapsedMs: Number(overlappingMs.toFixed(0)),
  });
  if (overlappingFail > 0) {
    warn(`${tag} overlapping sync: ${overlappingFail} doc(s) failed — continuing with remaining`);
    deps.inbox?.add({ kind: 'failed-docs', path: '', note: failedDocsNoticeMessage(overlappingFail) });
    new Notice(failedDocsNoticeMessage(overlappingFail), 8000);
  }
  console.debug(
    `${tag} overlapping done (${overlappingMs.toFixed(0)}ms): ${skippedClean} clean skipped, ${skippedHashMatch} dirty/hash skipped, ${overlappingFiles.length - skippedClean - skippedHashMatch - overlappingFail} synced, ${overlappingFail} failed`,
  );

  // 3. Recreated docs — local file exists at a path that still has a pending
  // delete intent. Push an explicit replace-tombstone doc_create instead of a
  // blind sync_push so the server can distinguish user recreate from stale
  // offline resurrection.
  tPhase = performance.now();
  if (mode !== 'pull') {
    for (const file of recreateFiles) {
      const content = await readEffectiveLocalContent(app, editor, file);
      contentHashes.set(file.path, fnv1aHash64(content));
      log(`${tag} recreate push`, { path: file.path, contentLen: content.length });
      const doc = await docs.getOrLoad(file.path);
      doc.sync_from_disk(content);
      push.consumePendingDeleteForRecreate(file.path);
      push.pushDocCreate(file.path, doc, { replaceTombstone: true });
    ...
    stepsDone += recreateFiles.length;
    onProgress?.(stepsDone, totalSteps, changed);
  }

  const recreateMs = performance.now() - tPhase;

  // 4. Local-only docs — push full snapshot via doc_create (skip in pull mode)
  tPhase = performance.now();
  if (mode !== 'pull') {
  ...
  for (const uuid of tombstoneSet) {
    ...
    // those pushes, so the path still shows as tombstone-only here — without
    // this skip the same run would trash what it just rescued.
    if (recreatePathSet.has(uuid)) continue;
  ...
  // 8. Clean orphaned .loro files (deleted/renamed docs, old encoding)
  tPhase = performance.now();
  ...
  deps.trace('initial-sync.phase-timings', {
    ...
    recreateMs: Number(recreateMs.toFixed(0)),
  ...
  console.debug(`${tag} initialSync complete (${(performance.now() - t0).toFixed(0)}ms)`);
}
...
async function syncOverlappingDoc(
  ...
  if (result) {
    ...
    if (result.delta.length > 0 && hadLocalDiskChange) {
      ...
      if (serverText.trim() !== '' && serverText !== localContent) {
        if (editedDuringStartup) {
          ...
          await writeServerText(deps, path, freshDoc.get_text(), localContent);
          await docs.persist(path);
          return { keepDirty };
        }
      }
    ...
    // 805-conflict-file storm in the a large real-world vault — see
    // the conflict-storm audit notes (local).
    if (
    ...
    if (localContent.trim() === '' && serverContent.trim() !== '' && !isActiveEditorDoc) {
      ...
      wroteToVault = true;
      await writeServerText(deps, path, serverContent, localContent);
    } else {
      ...
      if (isActiveEditorDoc) {
        ...
        if (editorAlreadyMatches) {
          ...
          try {
            const ops = JSON.parse(diffJson) as unknown;
          ...
          if (hasTextChanges) {
            if (editor.applyDiffToEditor(path, diffJson, serverContent, true)) {
              ...
              lastRemoteWrite.set(path, fnv1aHash64(postApplyContent ?? serverContent));
              keepDirty = true;
            } else {
