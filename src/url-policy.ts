...
export function validateServerUrl(raw: string): UrlValidation {
  ...
  const reason = forbiddenComponent(url);
 ...
 */
function forbiddenComponent(url: URL): string | null {
  if (url.username || url.password) return 'Server URL credentials are not allowed';
  if (url.href.split('#')[0].includes('?')) return 'Server URL query is not allowed';
  if (url.href.includes('#')) return 'Server URL fragment is not allowed';
  return null;
}

export function normalizeServerUrl(raw: string): string {
  // URL.canParse needs Chromium >= 95; Amazon WebView 84 (Fire tablets)
  // lacks it and the connect path dies with "TypeError: URL.canParse is not
  // a function" (device-tested 2026-09-06). Same compat class as
  // uuidV4Compat in settings.ts.
  const canParse = typeof URL.canParse === 'function'
    ? URL.canParse(raw)
    : (() => { try { new URL(raw); return true; } catch { return false; } })();
  if (canParse) {
    const reason = forbiddenComponent(new URL(raw));
